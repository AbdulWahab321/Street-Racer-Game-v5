...
if(gameState===DETECTER){
  background(first)
  if(gameState!==OTHERGAM){
  fill("red")
  textSize(40)
  text("Which is your System PC or Mobile",100,50);
text("____________________________________________________________",40,55)
 ...
 text("(PC)",com.x-10,com.y+10)
  if(mouseIsOver(mobile)){
    ...
    open("https://abdulwahab321.github.io/Car-Game-Mobile-Edition-v4/")
  }
  if(mouseIsOver(com)){
...
if(gameState!==FIRSTBG&&gameState!==DETECTER){
  moreG=createSprite(500,50,250,50);
  fill("white")
stroke("blue")
strokeWeight(4)
  text("More Games",moreG.x,moreG.y);
  moreG.visible=false;
  if(frameCount%10===0){
  if(mousePressedOver(moreG)){
...
if(gameState!==START&&gameState!==INS&&gameState!==FIRSTBG&&gameState!==DETECTER&&gameState!==OTHERGAM){
fill("white")
stroke("blue")
strokeWeight(4)
...
text("Lap: "+Lap,800,50)
  ...
  if(gameState!==FIRSTBG&&gameState!==DETECTER&&gameState!==OTHERGAM){
  fill("black")
stroke("black")
strokeWeight(2)
...
function updateStatus(event){
  ...
  if(navigator.onLine){
    ...
    if(c===true){
      var p= confirm("Welcome To Street Racer");
    }
